    ...
    return (
        <>
            <PageHeader
                eyebrow="Vendas"
                title="Pedidos"
                description="Acompanhe os pedidos gerados pelos checkouts."
            />
            <section className="mb-4 grid gap-3 sm:grid-cols-3">
                <SummaryCard
                    label="Volume de pedidos"
                    value={money(total, currency)}
                    detail={`${orders.length} pedidos gerados`}
                    icon="cart"
                />
                <SummaryCard
                ...
                <SummaryCard
                    ...
                    detail="Com pagamento integral"
                    icon="check"
                />
            </section>
            <ResourceTable
                title="Pedidos recentes"
                description="Histórico comercial dos seus checkouts"
                rows={orders}
                empty="Nenhum pedido encontrado."
                columns={[
                    { label: 'Pedido', value: (row) => row.id, mono: true },
                    { label: 'Cliente', value: (row) => row.customerId, mono: true },
                    { label: 'Status', value: (row) => row.status },
                    { label: 'Total', value: (row) => money(row.totalMinor, row.currency) },
                    { label: 'Pago', value: (row) => money(row.paidMinor, row.currency) },
                    { label: 'Data', value: (row) => date(row.placedAt) },
                ]}
            />
        </>
    );
