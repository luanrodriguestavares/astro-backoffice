...
export default async function DashboardPage() {
  ...
  const approved = payments.filter((payment) =>
    approvedStatuses.has(payment.status),
  );
  const revenue = approved.reduce(
    (sum, payment) => sum + capturedValue(payment),
    0,
  );
    ...
    <div className="dashboard-home">
      ...
      >
        <StatCard
          ...
          icon="card"
          href="/payments"
          sparkline={valuesWithActivity(
            recentTimeline.map((point) => point.value),
          )}
...
function recentActivities(input: {
  ...
  for (const payment of input.payments.filter((item) =>
...
function periodMetrics(payments: Payment[], offsetDays: number, days: number) {
  ...
  return {
    ...
    approvalRate: selected.length
      ? (approved.length / selected.length) * 100
      : 0,
...
function revenueTimeline(payments: Payment[], length: number) {
  ...
  for (const payment of payments) {
    const bucket = byDate.get(
      localDateKey(new Date(payment.approvedAt ?? payment.createdAt)),
    );
...
function paymentApprovalTimeline(payments: Payment[], length: number) {
  ...
  return all.map((total, index) =>
    total ? Math.round((approved[index] / total) * 100) : 0,
  );
}
