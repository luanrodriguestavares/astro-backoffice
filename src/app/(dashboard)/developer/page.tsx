...
export default async function DeveloperPage() {
    const keys = await apiFetch<ApiKey[]>('/api/v1/developer/api-keys');
    return (
        <>
            <PageHeader
                eyebrow="Integrações"
            ...
            <ResourceTable
                ...
                columns={[
                    ...
                    { label: 'Prefixo', value: (row) => row.prefix, mono: true },
                    { label: 'Escopos', value: (row) => row.scopes.join(', ') },
                    { label: 'Limite/min', value: (row) => row.rateLimitPerMinute },
                    { label: 'Status', value: (row) => row.status },
                    { label: 'Último uso', value: (row) => date(row.lastUsedAt) },
                ]}
            />
        </>
    );
