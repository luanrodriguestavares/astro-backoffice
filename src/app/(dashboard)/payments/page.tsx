  ...
  return (
    <>
      ...
      <section className="mb-4 grid gap-3 sm:grid-cols-3">
        <SummaryCard label="Volume recebido" value={money(received, currency)} detail={`${approved.length} pagamentos aprovados`} icon="card" />
        <SummaryCard label="Taxa de aprovação" value={percentage(payments.length ? approved.length / payments.length : 0)} detail={`${payments.length} tentativas processadas`} icon="chart" />
        <SummaryCard label="Total reembolsado" value={money(refunded, currency)} detail="Valores devolvidos aos clientes" icon="refund" />
      </section>
      ...
      <section className="glass-panel overflow-hidden rounded-[28px]">
        <div className="flex items-center justify-between gap-4 border-b border-white/65 px-5 py-5 sm:px-6">
          <div>
            <h2 className="text-sm font-semibold tracking-[-0.02em]">Transações recentes</h2>
            <p className="mt-1 text-[12px] text-muted">Pagamentos processados por todos os gateways</p>
          </div>
        ...
        {payments.length === 0 ? (
          <div className="px-5 py-14 text-center">
            ...
            <p className="mt-1 text-[13px] text-muted">As transações dos seus checkouts aparecerão aqui.</p>
          </div>
        ) : (
            ...
            <table className="w-full min-w-[900px] text-left">
              ...
              <tbody className="divide-y divide-white/65">
                {payments.map((payment) => {
                  ...
                  return (
                    <tr key={payment.id} className="group text-[13px] transition hover:bg-white/34">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-mono text-[12px] text-foreground">{payment.id}</span>
                        </div>
                      </td>
                      <td className="px-5 py-4">
                        <p className="font-semibold text-foreground">{customer?.name ?? "Cliente"}</p>
                        <p className="mt-1 text-[12px] text-muted">{customer?.email ?? payment.customerId}</p>
                      </td>
                      <td className="px-5 py-4 text-muted">{paymentMethod(payment.paymentMethod)}</td>
                      <td className="px-5 py-4 font-semibold tabular-nums">{money(payment.amountMinor, payment.currency)}</td>
                      <td className="px-5 py-4"><Status value={payment.status} /></td>
                      <td className="px-6 py-4 text-[12px] text-muted">{dateTime(payment.createdAt)}</td>
                    </tr>
                  );
                })}
              </tbody>
...
function Status({ value }: { value: string }) {
  ...
  return <span className={`inline-flex rounded-full border px-2.5 py-1 text-[11px] font-semibold capitalize ${good ? "border-emerald-100 bg-[#e8f7f1] text-success" : "border-amber-100 bg-[#fff3e5] text-warning"}`}>{value}</span>;
}
...
function paymentMethod(value: string) {
  return ({ pix: "Pix", card: "Cartão", credit_card: "Cartão de crédito", boleto: "Boleto" } as Record<string, string>)[value] ?? value;
}
