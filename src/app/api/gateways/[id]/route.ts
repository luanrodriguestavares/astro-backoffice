...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (input.credentials && Object.keys(input.credentials).length > 0)
            connection = await apiFetch<GatewayConnection>(
                `/api/v1/gateway-connections/${encodeURIComponent(id)}/rotate-credentials`,
                {
                    method: 'POST',
                    headers: { 'idempotency-key': randomUUID() },
        ...
        return NextResponse.json(
            { detail: 'Não foi possível atualizar o gateway.' },
            { status: 500 },
        );
    }
}
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        const { id } = await params;
        await apiFetch(`/api/v1/gateway-connections/${encodeURIComponent(id)}`, {
            method: 'DELETE',
        });
        ...
        return NextResponse.json(
            { detail: 'Não foi possível desabilitar o gateway.' },
            { status: 500 },
        );
    }
}
