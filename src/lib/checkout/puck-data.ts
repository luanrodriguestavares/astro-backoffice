...
export function documentToPuck(document: CheckoutDocument): BuilderData {
  ...
  return {
    root: { props: { themeMode: themeModeValue(theme.themeMode ?? theme.themePreset), grayTone: grayToneValue(theme.grayTone ?? theme.themePreset), fontFamily: fontValue(theme.fontFamily), backgroundColor: stringValue(theme.backgroundColor, "#f7f7fb"), surfaceColor: stringValue(theme.surfaceColor, "#ffffff"), textColor: stringValue(theme.textColor, "#202235"), accentColor: stringValue(theme.accentColor, "#7065e8"), radius: sizeValue(theme.radius), shadow: shadowValue(theme.shadow), maxWidth: widthValue(layout.maxWidth), componentGap: sizeValue(layout.componentGap), pagePadding: sizeValue(layout.pagePadding, "lg"), inputGroupStyle: inputGroupStyleValue(layout.inputGroupStyle) } },
    content: document.sections.filter((section) => section.visible && supported.has(section.type)).map((section) => ({ type: section.type, props: { ...migrateDefaultTitles(section.type, section.props), id: section.id } })),
  } as BuilderData;
}
...
export function puckToDocument(data: BuilderData, previous: CheckoutDocument): CheckoutDocument {
  ...
  return {
    schemaVersion: 1,
    theme: { themeMode: themeModeValue(root.themeMode), grayTone: grayToneValue(root.grayTone), fontFamily: fontValue(root.fontFamily), backgroundColor: safeColor(root.backgroundColor, "#f7f7fb"), surfaceColor: safeColor(root.surfaceColor, "#ffffff"), textColor: safeColor(root.textColor, "#202235"), accentColor: safeColor(root.accentColor, "#7065e8"), radius: sizeValue(root.radius), shadow: shadowValue(root.shadow) },
    layout: { ...previous.layout, maxWidth: widthValue(root.maxWidth), componentGap: sizeValue(root.componentGap), pagePadding: sizeValue(root.pagePadding, "lg"), inputGroupStyle: inputGroupStyleValue(root.inputGroupStyle) },
...
function inputGroupStyleValue(value: unknown): BuilderRootProps["inputGroupStyle"] { return value === "outlined" ? "outlined" : "filled"; }
