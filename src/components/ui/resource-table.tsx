    ...
    <section className="glass-panel overflow-hidden rounded-[28px]">
      {(title || description) && (
        <div className="flex items-center justify-between gap-4 border-b border-white/65 px-5 py-5 sm:px-6">
          <div>
            {title && <h2 className="text-sm font-semibold tracking-[-0.02em]">{title}</h2>}
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[760px] text-left">
            ...
            <tbody className="divide-y divide-white/65">
              {rows.map((row, index) => (
                <tr key={String((row as { id?: string }).id ?? index)} className="text-[13px] transition hover:bg-white/34">
                  {columns.map((column) => (
                    <td
                      key={column.label}
                      className={`px-6 py-4 ${column.mono ? "font-mono text-xs" : ""}`}
                    >
                      {column.render ? column.render(row) : formatResourceValue(column.value(row))}
                    </td>
                  ))}
                </tr>
              ))}
