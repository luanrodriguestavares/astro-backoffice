...
export function ResourceTableControls({ tableId, total, position }: { tableId: string; total: number; position: 'top' | 'bottom' }) {
    ...
    if (position === 'top') return (
        <div className="flex flex-col gap-3 border-b border-white/65 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <label className="relative block w-full sm:max-w-sm">
                ...
                <input value={state.query} onChange={(event) => update({ query: event.target.value.toLocaleLowerCase('pt-BR'), page: 1 })} placeholder="Buscar nesta lista" className="h-10 w-full rounded-xl border border-border bg-[var(--control-bg)] pl-10 pr-3 text-[13px] outline-none transition focus:border-brand/60" />
            </label>
            <div className="w-40">
                <span className="block">
                    <CustomSelect
                        ...
                        options={[10, 20, 50, 100].map((value) => ({ value: String(value), label: `${value} por página` }))}
                    />
                </span>
            </div>
        </div>
    ...
    return (
        <div className="flex flex-col gap-3 border-t border-white/65 px-5 py-4 text-[12px] text-muted sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <span>{start}–{end} de {matching}</span>
            <div className="flex items-center gap-2">
                <Button type="button" variant="icon" aria-label="Página anterior" title="Página anterior" disabled={currentPage === 1} onClick={() => update({ page: currentPage - 1 })}><Icon name="arrow-right" className="size-3.5 rotate-180" /></Button>
                <span className="px-2">Página {currentPage} de {pages}</span>
                <Button type="button" variant="icon" aria-label="Próxima página" title="Próxima página" disabled={currentPage === pages} onClick={() => update({ page: currentPage + 1 })}><Icon name="arrow-right" className="size-3.5" /></Button>
            </div>
        </div>
    );
}
...
function rows(tableId: string) {
    ...
    return [...document.querySelectorAll<HTMLTableRowElement>(`#${CSS.escape(tableId)} [data-resource-row]`)];
}
...
function apply(tableId: string, state: State) {
    const matching = rows(tableId).filter((row) => (row.dataset.search ?? '').includes(state.query));
