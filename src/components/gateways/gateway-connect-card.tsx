...
export function GatewayConnectCard({ gateway }: { gateway: GatewayDefinition }) {
    ...
    async function submit(event: FormEvent<HTMLFormElement>) {
        ...
        const response = await fetch('/api/gateways', {
            ...
            body: JSON.stringify(payload(gateway.provider, new FormData(event.currentTarget))),
        });
    ...
    return (
        <>
            ...
            >
                <GatewayMark
                    initials={gateway.initials}
                    color={gateway.color}
                    logo={gateway.logo}
                    logoFill={gateway.logoFill}
                />
                <span className="min-w-0 flex-1">
                    <span className="block truncate text-[12px] font-semibold">{gateway.name}</span>
                        ...
                        >
                            ...
                            <div className="mt-7 grid gap-4 sm:grid-cols-2">
                                <Field
                                    name="name"
                                    label="Nome"
                                    defaultValue={providerName(gateway.provider)}
                                />
                            ...
                            <div className="mt-7 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                                ...
                                <Button variant="primary" disabled={loading}>
                                    {loading ? 'Validando...' : 'Conectar gateway'}
                                </Button>
                            </div>
                        </form>
                    </div>,
                ...
                createPortal(
                    ...
                    >
                        <section
                            role="alertdialog"
                            aria-modal="true"
                            className="modal-surface glass-panel w-full max-w-lg rounded-[26px] p-6 shadow-[0_32px_100px_rgba(37,31,76,.24)]"
                        >
                            <span className="grid size-11 place-items-center rounded-2xl bg-[#e8f7f1] text-success">
                                <Icon name="check" />
                            </span>
                            <h3 className="mt-5 text-xl font-semibold">Gateway conectado</h3>
                            <p className="mt-2 text-[13px] leading-5 text-muted">
                                Copie agora os dados do webhook. O segredo não será exibido
                                novamente.
                            </p>
                            <Secret label="Endpoint" value={created.webhookUrl ?? ''} />
                            {created.webhookSecret && (
                                <Secret label="Segredo" value={created.webhookSecret} />
                            )}
...
export function GatewayEditModal({
    ...
    async function submit(event: FormEvent<HTMLFormElement>) {
        ...
        const response = await fetch(`/api/gateways/${connection.id}`, {
            ...
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify(
                editPayload(connection.provider, form, connection.publicConfiguration),
            ),
        });
            ...
            >
                <div className="flex items-start justify-between gap-5">
                    <div>
                        ...
                        <p className="mt-1.5 text-[13px] text-muted">
                            Atualize a identificação e, se necessário, substitua as credenciais.
                        </p>
                    </div>
                    <Button
                        type="button"
                ...
                <div className="mt-7 grid gap-4 sm:grid-cols-2">
                    <Field name="name" label="Nome" defaultValue={connection.name} />
                    <label className="text-[13px] font-semibold">
                        Ambiente
                        <div className="mt-2">
                            <CustomSelect
                    ...
                    <GatewayFields provider={connection.provider} connection={connection} />
                </div>
...
function GatewayFields({
    ...
    if (provider === 'stripe')
        return (
            <>
                <Field name="secretKey" label="Secret key" secret required={!optional} />
                <Field name="webhookSecret" label="Webhook secret" secret required={!optional} />
                <Field
                    name="publishableKey"
                    label="Publishable key"
                    defaultValue={stringConfig(connection, 'publishableKey')}
                />
                <Field
                    name="subscriptionProductId"
                    label="Product ID para assinaturas"
                    defaultValue={stringConfig(connection, 'subscriptionProductId')}
                />
            </>
        );
    if (provider === 'mercado_pago')
        return (
            <>
                <Field name="accessToken" label="Access token" secret required={!optional} />
                <Field name="webhookSecret" label="Webhook secret" secret required={!optional} />
                <Field
                    name="notificationUrl"
                    label="Notification URL"
                    defaultValue={stringConfig(connection, 'notificationUrl')}
                />
                <Field
                    name="subscriptionBackUrl"
                    label="URL de retorno"
                    defaultValue={stringConfig(connection, 'subscriptionBackUrl')}
                />
            </>
        );
    if (provider === 'abacate_pay')
        return (
            <>
                <Field name="apiKey" label="API key" secret required={!optional} />
                <Field name="webhookSecret" label="Webhook secret" secret required={!optional} />
            </>
        );
    return (
        <>
            <Field
                name="apiKey"
                label="API key"
                defaultValue={optional ? '' : 'development'}
                required={!optional}
            />
            <label className="text-[13px] font-semibold">
                Cenário
                <div className="mt-2">
                    <CustomSelect
                        name="scenario"
                        defaultValue={stringConfig(connection, 'scenario') || 'approve'}
                        options={[
...
function payload(provider: GatewayConnection['provider'], form: FormData) {
    ...
    if (provider === 'stripe') {
        credentials.secretKey = value('secretKey');
        credentials.webhookSecret = value('webhookSecret');
        publicConfiguration.publishableKey = value('publishableKey');
        publicConfiguration.subscriptionProductId = value('subscriptionProductId');
    }
    if (provider === 'mercado_pago') {
        credentials.accessToken = value('accessToken');
        credentials.webhookSecret = value('webhookSecret');
        publicConfiguration.notificationUrl = value('notificationUrl');
        publicConfiguration.subscriptionBackUrl = value('subscriptionBackUrl');
    }
    if (provider === 'abacate_pay') {
        credentials.apiKey = value('apiKey');
        credentials.webhookSecret = value('webhookSecret');
        publicConfiguration.paymentExpirationSeconds = 3600;
    }
    if (provider === 'mock') {
        credentials.apiKey = value('apiKey');
        credentials.scenario = value('scenario');
    }
    return {
        provider,
        name: value('name'),
...
function editPayload(
    ...
    const next = payload(provider, form);
    const primaryKey = {
        stripe: 'secretKey',
    ...
    const replaceCredentials = next.credentials[primaryKey] !== '';
    return {
        name: next.name,
        publicConfiguration: { ...currentConfiguration, ...next.publicConfiguration },
        credentials: replaceCredentials
            ? Object.fromEntries(
                  Object.entries(next.credentials).filter(([, value]) => value !== ''),
              )
            : {},
    };
}

function stringConfig(connection: GatewayConnection | undefined, key: string) {
    const value = connection?.publicConfiguration[key];
