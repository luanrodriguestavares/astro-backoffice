...
export function OrganizationForm({ organization }: { organization: Organization }) {
    ...
    async function submit(event: FormEvent<HTMLFormElement>) {
        ...
        if (!response.ok) {
            setFailed(true);
            setMessage(body.detail ?? 'Não foi possível salvar as configurações.');
            return;
        }
        ...
        <form onSubmit={submit} className="glass-panel rounded-[28px] p-6">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="mt-1 text-[12px] text-muted">
                        Identificação e preferências do painel
                    </p>
                </div>
            </div>
            <div className="mt-7 grid gap-4 sm:grid-cols-2">
                ...
                <Select
                    name="timezone"
                    label="Fuso horário"
                    defaultValue={organization.timezone ?? 'America/Fortaleza'}
                >
                ...
                <Select name="locale" label="Idioma" defaultValue={organization.locale ?? 'pt-BR'}>
            ...
            {message && (
                <p
                    role="status"
                    className={`mt-5 rounded-2xl border p-3 text-[13px] ${failed ? 'border-[#f7d8de] bg-[#fff5f7]/75 text-danger' : 'border-[#d5f0e5] bg-[#e8f7f1]/70 text-success'}`}
                >
                    {message}
                </p>
            )}
...
function Select({
    ...
    const options = Children.toArray(children)
        ...
        .map((option) => ({
            value: String((option.props as { value: string }).value),
            label: String((option.props as { children: React.ReactNode }).children),
        }));
    return (
        <label className="text-[13px] font-semibold">
            {label}
            <div className="mt-2">
                <CustomSelect
                    name={select.name ?? ''}
                    defaultValue={String(select.defaultValue ?? '')}
                    options={options}
                />
            </div>
        </label>
