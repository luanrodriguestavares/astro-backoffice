...
type EditorTarget =
  | { kind: "queue"; idea: RoadmapModerationIdea }
  | { kind: "published"; idea: RoadmapIdea };
...
export function CommunityRoadmap({
  ...
  const [submissionEditor, setSubmissionEditor] =
    useState<RoadmapSubmission | null>(null);
  ...
  useEscapeClose(
    suggestionOpen || Boolean(editor) || Boolean(submissionEditor),
    () => {
    if (!pending) {
      ...
      Object.fromEntries(
        stages.map((stage) => [
          ...
          board
            ...
            .sort(
              (a, b) =>
                b.likesCount - a.likesCount || a.position - b.position,
            ),
        ]),
  ...
  async function moveIdea(id: string, stage: RoadmapStage) {
    ...
    try {
      const response = await fetch(
        `/api/community-roadmap/${encodeURIComponent(id)}/move`,
        {
          method: "PATCH",
      ...
      setBoard((current) =>
        current.map((candidate) =>
          candidate.id === id ? payload.data! : candidate,
        ),
      );
      router.refresh();
    } catch (error) {
      ...
      setMessage({
        ...
        text:
          error instanceof Error
            ? error.message
            : "Não foi possível mover a sugestão.",
      });
    } finally {
      setMovingId(null);
      setDraggingId(null);
  ...
  async function toggleLike(id: string) {
      ...
      current.map((candidate) =>
        candidate.id === id
          ? {
              ...
              likesCount: Math.max(
                0,
                candidate.likesCount + (likedByMe ? 1 : -1),
              ),
            }
          : candidate,
      ),
    ...
    try {
      const response = await fetch(
        `/api/community-roadmap/${encodeURIComponent(id)}/like`,
        { method: "POST" },
      );
      const payload = (await response.json()) as {
      ...
      if (!response.ok || !payload.data)
        throw new Error(
          payload.detail ?? "Não foi possível registrar sua curtida.",
        );
      setBoard((current) =>
        current.map((candidate) =>
          candidate.id === id
  ...
  return (
    <div className="space-y-5">
      ...
      {activeTab === "roadmap" ? (
        ...
        <MySuggestions
          ideas={roadmap.mySubmissions}
          onEdit={setSubmissionEditor}
        />
      ) : (
        <ModerationQueue
      ...
      <div className="flex items-start justify-between gap-4 border-b border-border px-5 py-5 sm:px-6">
        <div>
          ...
          <p className="mt-1 text-[11px] leading-5 text-muted">
            Revise, ajuste o texto e aprove. Toda ideia aprovada entra primeiro no Backlog.
          </p>
        </div>
        <span
      ...
      {ideas.length === 0 ? (
        <div className="grid min-h-[330px] place-items-center px-6 text-center">
          <div className="max-w-sm">
            ...
            <h3 className="mt-4 text-[14px] font-semibold">Nenhuma ideia esperando análise</h3>
            <p className="mt-1.5 text-[11px] leading-5 text-muted">
              Quando um cliente enviar uma sugestão, ela aparecerá aqui antes de chegar ao quadro público.
            </p>
          </div>
        </div>
      ) : (
            ...
            >
              ...
              <div className="mt-auto flex items-center justify-between gap-3 border-t border-border pt-3">
                <div className="min-w-0">
                  <p className="truncate text-[10px] font-semibold">{idea.submitterName}</p>
      ...
      <div className="mt-3 flex min-h-8 items-center justify-between gap-3 border-t border-border/70 pt-2.5">
        ...
        >
          <Icon
            ...
            className={`size-3.5 ${
              idea.likedByMe ? "roadmap-heart-liked" : ""
            }`}
          />
      ...
      <section className="grid min-h-[420px] place-items-center rounded-[26px] border border-dashed border-border bg-surface-muted/35 p-8 text-center">
        <div className="max-w-sm">
          ...
          <p className="mt-1.5 text-[12px] leading-5 text-muted">
            Quando você compartilhar uma ideia, o andamento da análise
            aparecerá aqui.
          </p>
        </div>
      </section>
  ...
  return (
    <div className="flex items-start justify-between gap-5">
      <div>
        ...
        <p className="mt-1.5 max-w-lg text-[12px] leading-5 text-muted">
          {description}
        </p>
      </div>
      <Button
        ...
        <div className="mt-5 space-y-4">
          ...
          {forceBacklog ? (
            <div className="flex items-center gap-3 rounded-2xl border border-brand/12 bg-brand-soft/45 p-3.5">
              ...
              <div>
                <p className="text-[11px] font-semibold">Destino após aprovação: Backlog</p>
        ...
        <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-between">
          {target.kind === "queue" ? (
            <Button
              type="button"
              variant="danger"
              disabled={pending || title.trim().length < 5 || description.trim().length < 20}
