...
export function InviteMember() {
  ...
  return <><Button type="button" variant="primary" onClick={() => setOpen(true)}><Icon name="plus" className="size-3.5" />Convidar membro</Button>{open && createPortal(<div onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }} className="fixed inset-0 z-[100] grid place-items-center overflow-y-auto bg-[#17172c]/20 p-4 backdrop-blur-sm"><form onSubmit={submit} className="modal-surface glass-panel my-6 w-full max-w-lg rounded-[28px] p-5 shadow-[0_32px_100px_rgba(37,31,76,.2)] sm:p-7"><div className="flex items-start justify-between gap-5"><div><p className="text-[12px] font-semibold uppercase tracking-[0.16em] text-brand-strong">Acesso</p><h2 className="mt-2 text-2xl font-semibold tracking-[-0.04em]">Convidar membro</h2><p className="mt-1.5 text-[13px] leading-5 text-muted">Envie um convite e defina o nível de acesso à organização.</p></div><Button type="button" variant="icon" aria-label="Fechar" disabled={loading} onClick={close}><Icon name="close" className="size-4" /></Button></div>{token ? <><p className="mt-6 text-[13px] leading-5 text-muted">Em ambiente local, compartilhe este token com o convidado. Ele é exibido somente agora.</p><code className="mt-3 block overflow-x-auto rounded-xl border border-[#d9d7e8] bg-white/70 p-3 text-xs">{token}</code><div className="mt-6 flex justify-end"><Button type="button" variant="primary" onClick={close}>Concluir</Button></div></> : <><div className="mt-7 grid gap-4"><label className="text-[13px] font-semibold">E-mail<input name="email" type="email" required placeholder="nome@empresa.com" className="mt-2 h-11 w-full rounded-xl border border-[#d9d7e8] bg-white/70 px-3.5 font-normal outline-none transition placeholder:text-[#aaaabd] focus:border-brand/70 focus:bg-white focus:shadow-[0_0_0_3px_rgba(109,93,244,.16)]" /></label><label className="text-[13px] font-semibold">Função<div className="mt-2"><CustomSelect name="role" defaultValue="admin" options={[{ value: "admin", label: "Administrador" }, { value: "finance", label: "Financeiro" }, { value: "support", label: "Suporte" }, { value: "viewer", label: "Leitura" }]} /></div></label></div>{error && <p role="alert" className="mt-5 rounded-2xl border border-[#f7d8de] bg-[#fff5f7] p-3 text-[13px] text-danger">{error}</p>}<div className="mt-7 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button type="button" variant="secondary" disabled={loading} onClick={close}>Cancelar</Button><Button variant="primary" disabled={loading}>{loading ? "Enviando..." : "Enviar convite"}</Button></div></>}</form></div>, document.body)}</>;
}
