  ...
  useEffect(() => {
    ...
    function position() {
      ...
      setPopoverStyle({
        ...
        left,
        width,
        maxHeight: Math.max(240, Math.min(430, openAbove ? availableAbove : availableBelow)),
        zIndex: 160,
      });
    }
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <span className="mt-0.5 block text-[10px] text-muted">As alterações são aplicadas em tempo real.</span>
        </div>
        <Button type="button" aria-label="Fechar seletor" onClick={() => setOpen(false)} className="grid size-7 place-items-center rounded-lg text-muted hover:bg-surface-muted">
      ...
      <label className="mt-3 grid gap-1.5 text-[11px] font-semibold text-[#67677b]">
        ...
        <input
          ...
          onChange={(event) => updateDraft({ ...draft, h: Number(event.currentTarget.value) })}
        />
      </label>

      <div className="mt-3 grid grid-cols-[42px_minmax(0,1fr)] gap-2">
        <span className="rounded-xl border border-black/10" style={{ background: hsvToHex(draft) }} aria-hidden="true" />
      ...
      <div className="mt-3">
        <span className="text-[10px] font-semibold uppercase tracking-[.08em] text-[#77778a]">Cores rápidas</span>
  ...
  return (
    <FieldLabel label={label ?? "Cor"}>
      <div ref={root}>
        ...
        >
          <span className="size-7 shrink-0 rounded-lg border border-black/10 shadow-inner" style={{ background: selected }} />
          <span className="min-w-0 flex-1">
            <span className="block text-[9px] font-semibold uppercase tracking-[.08em] text-muted">Cor atual</span>
            <span className="block font-mono text-[12px] uppercase text-foreground">{selected}</span>
          </span>
...
function SaturationValue({ value, onChange }: { value: Hsv; onChange: (value: Hsv) => void }) {
  ...
  return (
    <div
      ...
      style={{ background: `linear-gradient(to top, #000, transparent), linear-gradient(to right, #fff, transparent), hsl(${value.h} 100% 50%)` }}
      onPointerDown={(event) => {
    ...
    >
      <span
        ...
        style={{ left: `${value.s * 100}%`, top: `${(1 - value.v) * 100}%`, background: hsvToHex(value) }}
      />
    </div>
  );
}
...
function hsvToHex({ h, s, v }: Hsv) {
  ...
  const m = v - chroma;
  let [r, g, b] = h < 60 ? [chroma, x, 0] : h < 120 ? [x, chroma, 0] : h < 180 ? [0, chroma, x] : h < 240 ? [0, x, chroma] : h < 300 ? [x, 0, chroma] : [chroma, 0, x];
