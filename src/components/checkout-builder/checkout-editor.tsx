          ...
          <>
            {(checkout.status === "published" || state === "published") && (
              <>
                ...
                >
                  Abrir checkout <Icon name="arrow-right" className="size-3.5 -rotate-45" />
                </a>
              </>
            )}
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <h2 className="text-[13px] font-semibold text-foreground">Não foi possível publicar</h2>
              <p className="mt-1.5 text-[12px] leading-5 text-muted">{message}</p>
            </div>
            <Button
              type="button"
              aria-label="Fechar aviso"
              onClick={() => { setState("changed"); setMessage(undefined); }}
...
function publishErrorMessage(code?: string, detail?: string) {
  if (code === "CHECKOUT_RUNTIME_COMPONENTS_MISSING") {
    const missing = detail?.split(":")[1]?.replace(".", "").split(",").map((item) => item.trim()).filter(Boolean) ?? [];
...
function joinLabels(labels: string[]) {
  ...
  return `${labels.slice(0, -1).map((label) => `“${label}”`).join(", ")} e “${labels.at(-1)}”`;
}
