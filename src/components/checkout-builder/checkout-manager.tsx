    ...
    <>
      ...
      {deleteTarget && createPortal(
        <div onMouseDown={(event) => { if (event.target === event.currentTarget && !deleting) setDeleteTarget(undefined); }} className="fixed inset-0 z-[110] grid place-items-center bg-[#17172c]/18 p-4 backdrop-blur-sm">
          <div className="theme-modal modal-surface glass-panel w-full max-w-md rounded-[26px] p-6 shadow-[0_30px_90px_rgba(37,31,76,.2)]" role="alertdialog" aria-modal="true" aria-labelledby="delete-checkout-title">
      ...
      {missingProductAlert && createPortal(
        <div onMouseDown={(event) => { if (event.target === event.currentTarget) setMissingProductAlert(false); }} className="fixed inset-0 z-[110] grid place-items-center overflow-y-auto bg-[#17172c]/18 p-4 backdrop-blur-sm">
          <section role="alertdialog" aria-modal="true" className="theme-modal modal-surface glass-panel my-6 w-full max-w-md rounded-[26px] p-6 shadow-[0_30px_90px_rgba(37,31,76,.2)]">
...
function CheckoutCard({ checkout, onDelete }: { checkout: Checkout; onDelete: () => void }) {
  ...
  return (
    <article className="checkout-list-card glass-panel group overflow-hidden rounded-[20px] p-3.5 transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_18px_48px_rgba(66,57,128,.08)]">
      <Link href={`/checkouts/${checkout.id}/builder`} className="relative block h-32 overflow-hidden rounded-[14px] border border-[#dfddea]/70 bg-gradient-to-br from-[#f5f3ff] to-[#edf4ff]" aria-label={`Abrir editor de ${checkout.name}`}>
        <span className="absolute inset-0 grid place-items-center text-brand/35"><Icon name="layout" className="size-7" /></span>
        <iframe
          src={previewUrl}
      ...
      <div className="mt-3.5 flex min-w-0 items-start justify-between gap-3 px-0.5">
        <div className="min-w-0">
          <h2 className="truncate text-[15px] font-semibold tracking-[-0.025em]">{checkout.name}</h2>
          {checkout.status === "published" ? (
            <a href={publicUrl} target="_blank" rel="noreferrer" className="mt-1 block max-w-full truncate text-[11px] text-brand-strong hover:underline">{publicUrl}</a>
          ) : (
            <p className="mt-1 truncate text-[12px] text-muted">/{checkout.slug}</p>
          )}
        </div>
        <span className={`checkout-list-status inline-flex h-6 shrink-0 items-center justify-center rounded-full border px-2.5 text-[10px] font-semibold leading-none ${checkout.status === "published" ? "border-emerald-100 bg-[#e8f7f1] text-success" : "border-[#dedce9] bg-white/55 text-muted"}`}>{checkout.status === "published" ? "Publicado" : "Rascunho"}</span>
      </div>
      ...
      <div className="checkout-list-footer mt-3.5 flex items-center justify-between border-t border-white/70 px-0.5 pt-3">
        ...
        <div className="flex items-center gap-1">
          <Button type="button" onClick={onDelete} aria-label={`Excluir ${checkout.name}`} className="grid size-8 place-items-center rounded-lg text-muted transition hover:bg-[#fff0f2] hover:text-danger"><Icon name="trash" className="size-3.5" /></Button>
          {checkout.status === "published" && (
            <Button type="button" onClick={() => void navigator.clipboard.writeText(publicUrl)} aria-label={`Copiar link público de ${checkout.name}`} title="Copiar link público" className="grid size-8 place-items-center rounded-lg text-muted transition hover:bg-brand-soft hover:text-brand-strong"><Icon name="link" className="size-3.5" /></Button>
          )}
          <Link href={`/checkouts/${checkout.id}/builder`} className="inline-flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-[12px] font-semibold text-brand-strong transition hover:bg-brand-soft">Editar <Icon name="arrow-right" className="size-3.5" /></Link>
        </div>
      </div>
    </article>
  );
